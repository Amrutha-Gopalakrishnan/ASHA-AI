...
export default function Testimonials() {
  ...
  return (
    <div className="w-full px-4 sm:px-8 md:px-16 py-10 bg-gradient-to-r from-purple-100 to-white text-center">
      <h2 className="text-2xl sm:text-3xl font-bold mb-10">
        Our <span className="text-purple-700 underline">Testimonials</span>
      </h2>

      {/* Avatar Row */}
      <div className="flex justify-center gap-4 sm:gap-6 md:gap-8 mt-6 sm:mt-10 items-center overflow-x-auto scrollbar-hide">
        {testimonials.map((t, index) => (
          <img
            key={index}
            src={t.image}
            alt={t.name}
            className={`rounded-full transition-all duration-300 border-purple-300 ${
              index === current
                ? "w-24 h-24 border-4 border-purple-500"
                : "w-14 h-14 opacity-40 border-2"
            }`}
          />
        ))}
      </div>
      ...
      >
        <p className="text-gray-700 mb-4 italic text-base sm:text-lg px-2 sm:px-0">
      ...
      <div className="mt-6 flex justify-center gap-6">
        ...
        >
          <FaRegHandPointLeft className="text-lg" />
        </button>
        <button
          onClick={handleNext}
          className="bg-purple-500 text-white px-4 py-2 rounded-full hover:bg-purple-600"
